...
export class PostsComponent {
  ...
  readonly contextMenuItems: MenuItem[] = [
    {
      ...
      command: () => {
        if (this.selectedPost) {
          void this.router.navigate(['/posts', this.selectedPost.id]);
        }
      },
    },
    {
  ...
  openEditDialog(post: IPost): void {
    const dialogRef = this.dialogService.open(PostEditDialogComponent, {
    ...
    dialogRef.onClose
      ...
      .subscribe((updatedPost: IPost | undefined) => {
        if (!updatedPost) {
          return;
        }
